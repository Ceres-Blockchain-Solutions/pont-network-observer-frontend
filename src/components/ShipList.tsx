    ...
    useEffect(() => {
        const fetchAllShipAccounts = async () => {
            try {
                ...
                // TODO:
                shipAccounts = shipAccounts.filter((shipAccount) => shipAccount.account.dataAccounts.length > 0);
                ...
                await Promise.all(shipAccounts.map(async (shipAccount, shipIndex) => {
                    if (shipAccount.account.dataAccounts.length === 0) {
                        accessArray[shipIndex] = false;
                        unapprovedArray[shipIndex] = false;
                    }

                    await Promise.all(shipAccount.account.dataAccounts.map(async (dataAccount) => {
                        const [externalObserversAccountAddr, _] = PublicKey.findProgramAddressSync(
                            [Buffer.from("external_observers_account"), dataAccount.toBuffer()],
                            program.programId
                        );

                        const externalObserversAccountData = await program.account.externalObserversAccount.fetch(externalObserversAccountAddr);

                        console.log("External Observers Account Data:", JSON.stringify(externalObserversAccountData, null, 2));

                        const hasAccess = externalObserversAccountData.externalObservers.some((observer) => observer.equals(publicKey));
                        const isUnapproved = externalObserversAccountData.unapprovedExternalObservers.some((unapprovedObserver) => unapprovedObserver.equals(publicKey));

                        if (hasAccess) {
                            accessArray[shipIndex] = true;
                            const index = externalObserversAccountData.externalObservers.findIndex((observer) => observer.equals(publicKey));
                            const masterKeyEncrypted = externalObserversAccountData.externalObserversMasterKeys[index];
                            ...
                            <tr key={index}>
                                ...
                                <td>
                                    {account.account.dataAccounts.length > 0 && (
                                        hasAccess[index] ? (
                                            <button onClick={async () => await handleViewData(account.account.ship.toString())}>View Data</button>
                                        ) : (
                                            unapprovedExternalObservers[index] ? (
                                                <span>Already Requested</span>
                                            ) : (
                                                <button onClick={() => handleRequestAccess(index)}>Request Access</button>
                                            )
                                        )
                                    )}
                                </td>
